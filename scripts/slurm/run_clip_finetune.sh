...
#SBATCH --output=artifacts/logs/clip_ft_%j.out
#SBATCH --error=artifacts/logs/clip_ft_%j.err
#SBATCH --gres=gpu:1
...
export UV_PROJECT_ENVIRONMENT="${UV_PROJECT_ENVIRONMENT:-$AGILE_WM_ARTIFACTS_ROOT/cache/venvs/clip-ft-py310}"
...
DEFAULT_DATA_ROOT="$AGILE_WM_ARTIFACTS_ROOT/datasets/frame_caption_pairs_temp"
if [[ ! -d "$DEFAULT_DATA_ROOT" ]] || ! compgen -G "$DEFAULT_DATA_ROOT/$SHARD_GLOB" > /dev/null; then
  DEFAULT_DATA_ROOT="$AGILE_WM_ARTIFACTS_ROOT/datasets/frame_caption_pairs"
fi

DATA_ROOT="${DATA_ROOT:-$DEFAULT_DATA_ROOT}"
OUTPUT_DIR="${OUTPUT_DIR:-$AGILE_WM_ARTIFACTS_ROOT/models/clip_finetune}"
LOCAL_DATA_DIR="${LOCAL_DATA_DIR:-${SLURM_TMPDIR:-/tmp}/clip_caption_shards}"
CLIP_MODEL="${CLIP_MODEL:-openai/clip-vit-base-patch32}"

ensure_artifact_dirs "$OUTPUT_DIR" "$(dirname "$UV_PROJECT_ENVIRONMENT")"

if [[ ! -d "$DATA_ROOT" ]]; then
  echo "Configured DATA_ROOT does not exist: $DATA_ROOT" >&2
  echo "Set DATA_ROOT explicitly or point AGILE_WM_ARTIFACTS_ROOT at the artifact tree you want to use." >&2
  exit 1
fi
