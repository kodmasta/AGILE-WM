...
#SBATCH --output=logs/rnn_train_%j.out
#SBATCH --error=logs/rnn_train_%j.err
#SBATCH --gres=gpu:1
...
DEFAULT_ROLLOUT_DIR="$SCRATCH/my_dataset/rollouts"
if [[ ! -d "$DEFAULT_ROLLOUT_DIR" ]]; then
  DEFAULT_ROLLOUT_DIR="$SCRATCH/my_datasets"
fi
ROLLOUT_DIR="${ROLLOUT_DIR:-$DEFAULT_ROLLOUT_DIR}"
ROLLOUT_GLOB="${ROLLOUT_GLOB:-episode_*.npz}"

RESULTS_ROOT="${RESULTS_ROOT:-$SCRATCH/AGILE-WM/results/$EXP_NAME/$ENV_NAME}"
SERIES_DIR="${SERIES_DIR:-$RESULTS_ROOT/series}"
RNN_DIR="${RNN_DIR:-$RESULTS_ROOT/tf_rnn}"

RECONSTRUCTION_DIR="${RECONSTRUCTION_DIR:-$PWD/fusion_reconstruction_runs/shard0_ep5_cbp544}"
...
mkdir -p logs "$HF_HOME" "$TRANSFORMERS_CACHE" "$(dirname "$JOB_ENV_ROOT")" "$RESULTS_ROOT"
...
choose_clip_checkpoint() {
  ...
  local candidates=(
    "$SCRATCH/AGILE-WM/clip_finetune/merged_final"
    "$SCRATCH/AGILE-WM/clip_finetune/lora_final"
    "$HOME/clip_finetune/merged_final"
    "$HOME/clip_finetune/lora_final"
    "$PWD/merged_final"
    "$PWD/lora_final"
  )
  local candidate
...
install_torch_stack() {
  ...
  case "$stack_name" in
    legacy-cu124)
      ...
      ;;
    modern-default)
      uv pip install --python "$python_bin" "${MODERN_TORCH_PACKAGES[@]}"
      ;;
    *)
