...
class MDNRNN(tf.keras.Model):
    def __init__(self, args):
        ...
        self.inference_base.build((None, args.rnn_max_seq_len, args.rnn_input_seq_width))
        self.out_net.build((None, args.rnn_size))
...
def rnn_init_state(rnn):
  return [
      tf.zeros((1, rnn.args.rnn_size), dtype=tf.float32),
      tf.zeros((1, rnn.args.rnn_size), dtype=tf.float32),
  ]
